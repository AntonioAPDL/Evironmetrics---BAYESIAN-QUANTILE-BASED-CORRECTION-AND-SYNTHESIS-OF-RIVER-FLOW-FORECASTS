...
\]
with \(\mathbf{G}_t\) repeated once for the latent trend/seasonal block and once for each discrepancy block, and
\[
...
\end{align}
The vector \(\mathbf{e}_{T+k,j}\) is the forecast-window analogue of \(\mathbf{h}_{t,j}\): it extracts the latent trend/seasonal block, the discrepancy block for source \(j\), and, when active, the transfer block. The matrix \(\boldsymbol{M}_{T+k}\) propagates the same components forward into the forecast window. In the full specification used by exAL-M-T1, the transfer states \((\zeta_t,\boldsymbol{\psi}_t)\) remain active and the forecast covariates available at the cutoff are propagated through the transition block. The reduced T0 variant is obtained by suppressing this forecast-window transfer block after the cutoff.
